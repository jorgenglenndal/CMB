...
const char *SPLINE_ERROR_lin = "Error in spline routines\n";
...
Spline_lin::Spline_lin(std::string name) : name(name) {}

//====================================================
// Assignment constructor
//====================================================
Spline_lin& Spline_lin::operator=(const Spline_lin& rhs){
...
// Copy constructor
//====================================================
Spline_lin::Spline_lin(const Spline_lin& rhs){
  // We just create the spline from scratch instead of copying all the data
  if(rhs.spline)
    create(rhs.spline->x, rhs.spline->y, rhs.size_x, rhs.name, rhs.interpoltype_used);
  else
...
double Spline_lin::operator()(double x) const{
  return eval(x);
}

double Spline_lin::eval(const double x) const{
  if(!spline) {
...
double Spline_lin::eval_deriv(const double x, const int deriv) const{
  if(!spline) {
...
//====================================================
void Spline_lin::out_of_bounds_check(const double x) const{
...
double Spline_lin::deriv_x(const double x) const{
  return eval_deriv(x, 1);
}
double Spline_lin::deriv_xx(const double x) const{
  return eval_deriv(x, 2);
}
std::pair<double,double> Spline_lin::get_xrange() const{
  return {xmin, xmax};
}
std::string Spline_lin::get_name() const{
  return name;
}
void Spline_lin::set_out_of_bounds_warning(bool v){
